...
class EmpresaDAO{
	...
	function cadastrar(Empresa $o_empresa){
		...
		$this->sql = sprintf("INSERT INTO empresa (nomeFantasia, razaoSocial, nomeReduzido, CNPJ, inscricaoEstadual, inscricaoMunicipal, endereco, numero, complemento, bairro, cep, imagemLogotipo, login, datasis, idlocalidade, idimposto) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d, %d)",
				...
				mysqli_real_escape_string($this->con, $o_empresa->getLogin()),
				mysqli_real_escape_string($this->con, $o_empresa->getDatasis()),
				mysqli_real_escape_string($this->con, $o_empresa->getIdlocalidade()),
				mysqli_real_escape_string($this->con, $o_empresa->getIdimposto()));
	...
	function atualizar(Empresa $o_empresa){
		$this->sql = sprintf("UPDATE empresa SET nomefantasia= '%s', razaoSocial= '%s', nomeReduzido= '%s', CNPJ= '%s', inscricaoEstadual='%s', inscricaoMunicipal='%s', endereco='%s', complemento='%s', bairro='%s', cep='%s', imagemLogotipo='%s', login='%s', datasis='%s', idlocalidade=%d, idimposto=%d WHERE id= %d",
				mysqli_real_escape_string($this->con, $o_empresa->getNomeFantasia()),
				mysqli_real_escape_string($this->con, $o_empresa->getRazaoSocial()),
				mysqli_real_escape_string($this->con, $o_empresa->getNomeReduzido()),
				mysqli_real_escape_string($this->con, $o_empresa->getCNPJ()),
	...
	function deletar(Empresa $o_empresa){
		$this->sql = sprintf("DELETE FROM empresa WHERE id = %d",
				mysqli_real_escape_string( $this->con, $o_empresa->getId() ) );
		if(!mysqli_query($this->con, $this->sql)){
	...
	function buscarPorID(Empresa $o_empresa){
		$this->sql = sprintf("SELECT * FROM empresa WHERE id = %d",
				mysqli_real_escape_string( $this->con, $o_empresa->getId() ) );
	...
	/*-- Listar Todos --*/
	function listarTodos(Empresa $o_empresa){
		...
		$this->o_empresa = new Empresa($row->id, $row->nomeFantasia, $row->razaoSocial, $row->nomeReduzido, $row->CNPJ, $row->atinscricaoEstatuasl, $row->inscricaoMunicipal, $row->endereco, $row->numero, $row->complemento, $row->bairro, $row->cep, $row->imagemLogotipo, $row->login, $row->datasis, $row->idlocalidade, $row->idimposto); 
		}
	...
	function listarPorNome(Empresa $o_empresa){
		...
		$this->sql = sprintf("SELECT * FROM empresa WHERE nomeFantasia like '%s%s%s' ",
